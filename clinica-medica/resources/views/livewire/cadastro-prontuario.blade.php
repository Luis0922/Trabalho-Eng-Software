    ...
    <x-card class="mb-4">
        <form method="POST" action="{{ route('login') }}">
            ...
            <div>
                ...
                <x-textarea id="anamnese" class="block mt-1 w-full" type="text" name="anamnese" required />
            </div>
            <div class="mt-4">
                <x-label for="medicamentos" value="{{ __('Prescrição de medicamentos') }}" />
                <x-textarea id="medicamentos" class="block mt-1 w-full" type="text" name="medicamentos" required />
            </div>
            <div class="mt-4">
                <x-label for="atestados" value="{{ __('Emissão de atestados') }}" />
                <x-textarea id="atestados" class="block mt-1 w-full" type="text" name="atestados" required />
            </div>
            <div class="mt-4">
                <x-label for="exames" value="{{ __('Solicitação de exames') }}" />
                <x-textarea id="exames" class="block mt-1 w-full" type="text" name="exames" required />
            </div>
